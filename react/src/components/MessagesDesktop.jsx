...
function renderMessageWithLinks(text) {
  ...
  return parts.map((part, i) => {
    if (urlRegex.test(part)) {
      return (
        <a key={i} href={part} target="_blank" rel="noopener noreferrer" style={{ color: '#320A6B', textDecoration: 'underline', wordBreak: 'break-all' }}>{part}</a>
      );
    }
...
const MessagesDesktop = ({
  ...
  return (
     <>
          <aside className="w-80 border-r border-[#23272f] bg-[#16181C] flex-shrink-0">
            ...
            {/* Search bar at top of aside (desktop only) */}
            ...
            <ul>
              {(() => {
                // Fix: Always show messaged users with non-empty chats in sidebar
                let usersToShow = [];
                if (searchTerm.trim()) {
                  ...
                  // Sort by most recent message date (like mobile), then unread count
                  usersToShow = sortedUsers
                    ...
                    .sort((a, b) => {
                      // Sort by most recent message date desc, then unread count desc
                      ...
                      >
                        ...
                        <div className="relative flex flex-col">
                          <div className="font-semibold flex items-center gap-1" style={{display:'flex',flexDirection:'row',alignItems:'center'}}>
                            ...
                            {/* Show unread badge for both desktop and mobile */}
                            {unread > 0 && (
          ...
          <main className="flex-1 flex flex-col">
            <header className="p-4 border-b border-[#23272f] bg-[#16181C] font-bold text-lg flex items-center gap-2 whitespace-nowrap overflow-hidden relative">
              {selectedUser ? (
                <span className="flex items-center gap-2 min-w-0">
                  {selectedUser.profilePhoto && selectedUser.profilePhoto.url ? (
                    <img
                  ...
                  <span className="flex items-center gap-1 text-ellipsis overflow-hidden min-w-0">
                    <span className="truncate max-w-[120px]">{selectedUser.username}</span>
              ...
              {/* Desktop 3-dot menu (same as mobile, but in header right) */}
              {selectedUser && (
                <div style={{ marginLeft: 'auto', position: 'relative', zIndex: 2000 }}>
                  ...
                  {showHeaderMenuMobile && (
                    <div
                      style={{
                        position: 'fixed',
                        top: 70, // header height for desktop
                        right: 36,
                        background: '#23272f',
                        color: '#fff',
                        borderRadius: 10,
                    ...
                    >
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        {/* Block/Unblock user button logic */}
            ...
            <div
              ...
              style={{
                maxHeight: 'calc(100vh - 136px)', // header (56px) + input (60px) + padding
                minHeight: 200,
                height: '100%',
                overflowY: 'auto',
                scrollbarWidth: 'thin',
                          ...
                          >
                            ...
                            >
                              {(isSender && editingMsgIdx === idx) ? (
                                ...
                                <span>{renderMessageWithLinks(msg.content)}</span>
                              )}
                                ...
                                >
                                  ...
                                  {isSender && (
                                    ...
                                    onClick={e => {
                                      ...
                                      setOpenDropdownIdx(null);
                                      setActiveMsgIdx(null);
                                      // For desktop, pass both idx and message id to modal
            ...
            {/* Down arrow button just above the input (fixed for mobile) */}
            {showScrollToBottom && (
              <div
                style={
                  isMobile
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    margin: isMobile ? '0 auto' : '8px auto 8px auto',
                    pointerEvents: 'auto',
                  }}
            ...
            <form className="p-4 border-t border-[#23272f] bg-[#16181C] flex" onSubmit={handleSend}>
              <input
                type="text"
